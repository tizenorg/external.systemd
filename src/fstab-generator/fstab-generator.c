...
static bool hasfs_comment(
        ...
        comment = strstr(mntopts, COMMENT_HASFS);
        if (!comment)
                return false;
        ...
        fs = comment + strlen(COMMENT_HASFS);
...
#ifdef CONFIG_TIZEN_WIP
                        /* TODO */
                        /* Tizen is now using switch-able smack
                         * feature. On feature enabled, different
                         * mount option is needed for tmpfs. This
                         * problematic problem is solved in recent
                         * util-linux. But we are not using mount of
                         * util-linux. So to compat the both case, the
                         * smackfsroot=* option will be conditionally
                         * be parsed by fstab.
                         */
                        if (me->mnt_opts) {
                                _cleanup_free_ char *fs_new = NULL;
                                char *comment_start, *comment_end, *opt;
                                int n;

                                opts = strdup(me->mnt_opts);
                                assert(opts);

                                if (hasfs_comment(opts, &comment_start, &comment_end, &opt, &fs_new)) {
                                        if (fs_new && use_smack())
                                                memmove(comment_start, opt, strlen(opt) + 1);
                                        else
