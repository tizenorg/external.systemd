...
/* havefs-fsname-option */
static const char *str_comment_havefs = "comment=havefs-";

static bool havefs_comment(const char *mntopts, char **comment_start,
			   char **comment_end, char **opt_start, char **fs_new)
{
        char *comment, *fs, *opt, *end;

        /* comment=havefs-foo-opt=1 */

        comment = strstr(mntopts, str_comment_havefs);
        if (!comment)
                return false;
        ...
        if (comment_end)
                *comment_end = end ? end + 1 : comment + strlen(comment);

        fs = comment + strlen(str_comment_havefs);
...
static int add_mount(
        ...
        if (!isempty(opts) &&
            !streq(opts, "defaults")) {
                char _cleanup_free_ *parsed_opts = NULL, *fs_new = NULL;
                char *comment_start, *comment_end, *fs_opt;

                parsed_opts = strdup(opts);
                assert(parsed_opts);

                if (havefs_comment(parsed_opts, &comment_start, &comment_end, &fs_opt, &fs_new)) {
                        int n;

                        if (fs_new && fs_available(fs_new))
                                memmove(comment_start, fs_opt, strlen(fs_opt) + 1);
                        else
                                memmove(comment_start, comment_end, strlen(comment_end) + 1);

                        n = strlen(parsed_opts);
                        if (n > 0 && parsed_opts[n - 1] == ',')
                                parsed_opts[n - 1] = '\0';
                }
