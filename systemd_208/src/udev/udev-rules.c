                    ...
                    const char *filename, unsigned int filename_off, unsigned int lineno)
{
        ...
        for (;;) {
                ...
                /* TODO */
                /* To keep the compatibility with old style udev smack
                 * rule, we will support also old smack
                 * option("SMACK=") until all of rules be changed to
                 * newly added udev smack rule
                 * style("SECLABEL{smack}="). */
                if (startswith(key, "SMACK")) {
                        rule_add_key(&rule_tmp, TK_A_SECLABEL, op, value, "smack");
                        continue;
                }
#endif
