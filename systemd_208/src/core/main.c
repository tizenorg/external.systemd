...
static uint64_t arg_default_memory_limit = 0;
...
static int parse_config_file(void) {
        ...
        const ConfigTableItem items[] = {
                ...
                { "Manager", "DefaultMemoryLimit",    config_parse_bytes_size,   0, &arg_default_memory_limit },
...
int main(int argc, char *argv[]) {
        ...
        if (getpid() == 1 && detect_container(NULL) <= 0) {
                ...
                if (!skip_setup) {
                        mount_setup_early();
                        /* If systemd was boot as proc 1 tizen-boot.sh will be
                         * called for pre-mount operation. */
        ...
        m->memory_limit = arg_default_memory_limit;
