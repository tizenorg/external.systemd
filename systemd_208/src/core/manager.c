...
int manager_load_unit_prepare(
        ...
        if (m->memory_limit && t == UNIT_SERVICE)
                SERVICE(ret)->cgroup_context.memory_limit = m->memory_limit;
...
int manager_set_default_extra_dependencies(Manager *m, char **dependencies) {
        ...
        d = strv_merge(m->dependencies, dependencies);
        if (!d)
                return -ENOMEM;
